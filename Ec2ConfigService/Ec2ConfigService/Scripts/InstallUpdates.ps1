...
function EC2ConfigPackage
{
	...
	switch -wildcard ((Get-WmiObject Win32_OperatingSystem).Caption)
	{
		"*2008*"	#Windows 2008
		{
			...
			remove-item -Path "C:\Users\Administrator\AppData\Roaming\Microsoft\Windows\Recent\*" -Force -Recurse -Confirm:$false
			remove-item -Path "C:\Users\Administrator\AppData\Local\Temp\*" -Force -Recurse -Confirm:$false
		...
		"*Server 2003*"	#Windows 2003
		{
			...
			#Cleanup Profile Usage Information
			remove-item -Path "C:\Documents and Settings\Administrator\Recent\*" -Force -Recurse -Confirm:$false
			remove-item -Path "C:\Documents and Settings\Administrator\Local Settings\Temp\*" -Force -Recurse -Confirm:$false
	...
	remove-item -Path "C:\Windows\Temp\*" -Force -Recurse -Confirm:$false

	#Removes old EC2Config Log
	remove-item -Path "C:\Program Files\Amazon\Ec2ConfigService\Logs\Ec2ConfigLog.txt" -Force -Confirm:$false
	...
	remove-item -Path "C:\Windows\*.DMP" -Force -Confirm:$false
	remove-item -Path "C:\Windows\Minidump\*.DMP" -Force -Confirm:$false
	...
	#Gets the content of EC2Config Settings and enables the Password element so new password is generated on next boot
